...
import ImagePreviewVue from 'packages/image-preview/src/image-preview';
...
describe('ImagePreview', () => {
  ...
  it('call ImagePreview Function', (done) => {
    ImagePreview(images);
    Vue.nextTick(() => {
      expect(document.querySelectorAll('.vu-image-preview img').length).to.equal(3);
      ImagePreview(images.slice(0, 2));

      Vue.nextTick(() => {
        expect(document.querySelectorAll('.vu-image-preview img').length).to.equal(2);
        done();
      });
    });
  });
  ...
  it('create a ImagePreview Component', (done) => {
    ...
    expect(wrapper.hasClass('vu-image-preview')).to.be.true;
