...
describe('Tabs', () => {
  ...
  it('create a empty card tabs', () => {
    ...
    expect(wrapper.hasClass('vu-tabs')).to.be.true;
    expect(wrapper.hasClass('vu-tabs--card')).to.be.true;
  });
  ...
  it('create a tabs with four tab', (done) => {
    ...
    expect(wrapper.hasClass('vu-tabs')).to.be.true;
    expect(wrapper.hasClass('vu-tabs--line')).to.be.true;

    const tabsContainer = wrapper.find('.vu-tabs')[0];
  ...
  it('listen click event', (done) => {
    ...
    wrapper.vm.$nextTick(() => {
      const nTab = wrapper.find('.vu-tab')[0];
      nTab.trigger('click');
  ...
  it('listen click disable event', (done) => {
    ...
    wrapper.vm.$nextTick(() => {
      const nTab = wrapper.find('.vu-tab')[2];
      nTab.trigger('click');
  ...
  it('create a tabs greater then 4', (done) => {
    ...
    wrapper.vm.$nextTick(() => {
      const nTab = wrapper.find('.vu-tab')[4];
      nTab.trigger('click');
      done();
    });
  });
  ...
  it('create a tabs greater then 4 then click last tab', (done) => {
    ...
    wrapper.vm.$nextTick(() => {
      const nTab = wrapper.find('.vu-tab')[6];
      nTab.trigger('click');
      done();
    });
  });
  ...
  it('watch tab props changes', (done) => {
    ...
    wrapper.vm.$nextTick(() => {
      expect(wrapper.find('.vu-tab')[0].text().replace(/\n|\s/g, '')).to.equal('测试标题');
      done();
    });
  });
